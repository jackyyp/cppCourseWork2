...
HashTable<K,V>::HashTable(unsigned int table_size, 
              ...
              :table_size(table_size),hash_func(hash_func),hash_func2(hash_func2),algo(algo){
...
unsigned int HashTable<K,V>::insert(const K& key, const V& value) {
  ...
  while(i < table_size * 2) { // Assume the number of probes should be less than 2*table_size
    ...
    if(table[j].status==ACTIVE){
       // Key already exists in table, update value
      if(table[j].key==key){
        table[j].value = value;
        return i;
      }
    ...
    if(table[j].status==EMPTY ||table[j].status==DELETE){ //we can insert to deleted location
    // Key not exists in table, add key-value, also set the status to ACTIVE
      table[j].key = key;
      table[j].value = value;
