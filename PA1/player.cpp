...
PlayerList::PlayerList(const PlayerList& list){
    ...
    this->numPlayers = list.getNumPlayers(); // size of array

    this->players = new Player*[this->numPlayers];
    for(int i=0;i<this->numPlayers;i++){
        this->players[i]  = list.getPlayer(i); //assign by element.
    }
}
...
void PlayerList::addPlayer(Player* const player){
    ...
    temp[this->numPlayers] = player; // add the new player* at end
...
void PlayerList::sort(){
        ...
        for(int i =1 ; i<this->getNumPlayers();i++){ // if i=1 , it wont run the for loop.
            int j = i-1;

            while(j>=0 && this->getPlayer(j)->getELO() < this->getPlayer(j+1)->getELO()){ //sort desc.
                //swap the *value* of the 2 corresponding pointers in the array.
        ...
        //sort by score , as We call sort() again.
