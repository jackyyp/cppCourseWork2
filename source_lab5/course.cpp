...
bool Course::operator<(const Course& other) const{
    if(code.compare(other.code)<0){return true;}
    return false;
}
...
istream& operator>>(istream& is, Course& c){
    // string temp;
    // is>>temp;
    // const string temp2=temp; //brute force  ,naive

    const string temp  = [&is]()->string{string t;is>>t;return t;}(); // using lambda

    c.set_code(temp); //mutator , error case checked by set_code()
    return is;
}
