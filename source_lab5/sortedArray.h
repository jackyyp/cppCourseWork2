...
template <class T, int MAX_SIZE, bool ASCENDING> // read only, RBV
...
bool SortedArray<T,MAX_SIZE,ASCENDING>::operator==(const SortedArray& other) const{
    if(num_elements!=other.size()){return false;} //diff size
...
template <class T, int MAX_SIZE, bool ASCENDING>
SortedArray<T, MAX_SIZE, ASCENDING> operator+(const SortedArray<T, MAX_SIZE, ASCENDING>& sa1,const SortedArray<T, MAX_SIZE, ASCENDING>& sa2){
    SortedArray<T, MAX_SIZE, ASCENDING> temp(sa1); //default copy ctor
    for(int i =0;i<sa2.size();i++){
        temp.insert(sa2[i]);//edge case are handled. just insert all
    }
